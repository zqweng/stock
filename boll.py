...
"""
   ...
   be a huge change. 
"""
def first_cross_above_upper(df1=None, market='usa', period='day', offset=0, silence=5, rise=0):
    myapi.market = market
    lib2.head_offset = offset
    if rise >= 0:
        df = dr5.get_a_across_b(df=df1, period_of_days=1, cross_above=("close", "upper", rise), cross_type="binary-cmp",
                                period_type=period)
    else:
        df = dr5.get_a_across_b(df=df1, period_of_days=1, cross_above=("close", "upper"), cross_type="binary-cmp",
                            period_type=period)
    lib2.head_offset = 1 + offset
