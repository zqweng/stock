...
def day_how_deep_is_shade(row, stock_row):
    ...
    shade_ratio = (row.open - row.low)/(row.close - row.open)


    #print('code:', stock_code, ' ratio:', shade_ratio, ' date:', row.date, ' open:', row.open, ' close:', row.close,\
    #' low:', row.low, ' ma5', row.ma5, ' ma10', row.ma10, ' ma20:', row.ma20)
    if shade_ratio > 2:
...
def day_break_moving_avg(row, stock_row):
    ...
    if close_above:
         if open_below or low_below:
              print('code:', stock_code, 'date:', date_val, ' open:', row.open, ' close:', row.close,\
                    ' low:', row.low, ' ma5', row.ma5, ' ma10', row.ma10, ' ma20:', row.ma20)

              volume_list.append(tuple((stock_row.name, date_val, stock_code)))
